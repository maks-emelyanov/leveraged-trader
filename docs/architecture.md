...
Asset-level work is scheduled through `asyncio` as a bounded producer/consumer pipeline with blocking Yahoo Finance, Tradier, and SQLite calls isolated in worker threads. `--workflow-concurrency` controls a fixed pool of asset download workers, which feed a capacity-one prepared-data queue to a single SQLite strategy consumer; `1` retains fully serial behavior. Yahoo Finance calls remain internally serialized because yfinance exposes process-global error and logger state, while parsing, Tradier fallback, and downloads can still overlap the SQLite consumer. Signal and risk-free history caches are populated under narrow locks, and SQLite strategy processing remains one-at-a-time because a shared benchmark correction can invalidate every strategy. Each processing unit also uses a SQLite `BEGIN IMMEDIATE` transaction plus a persisted state generation check, so independent processes cannot commit stale dependent state after an invalidation. Alpaca reconciliation and submission run after the asset pipeline through their existing worker-thread boundaries. Individual asset failures remain visible in the summary, but a run with no completed assets raises a workflow error before reports or buy submission.
