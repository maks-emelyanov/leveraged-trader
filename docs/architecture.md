...
Asset-level work is scheduled through `asyncio` as a bounded producer/consumer pipeline with blocking Yahoo Finance and Tradier preparation calls isolated in a dedicated download worker pool. `--workflow-concurrency` controls that fixed pool of asset download workers, which feed a capacity-one prepared-data queue to a single SQLite strategy consumer; `1` retains fully serial behavior. SQLite strategy processing runs outside the download pool and remains one-at-a-time because a shared benchmark correction can invalidate every strategy. Yahoo Finance calls remain internally serialized because yfinance exposes process-global error and logger state, while parsing, Tradier fallback, and downloads can still overlap the SQLite consumer. Signal and risk-free history caches are populated under narrow locks. Each processing unit also uses a SQLite `BEGIN IMMEDIATE` transaction plus a persisted state generation check, so independent processes cannot commit stale dependent state after an invalidation. Alpaca reconciliation and submission run after the asset pipeline through their existing worker-thread boundaries. Individual asset failures remain visible in the summary, but a run with no completed assets raises a workflow error before reports or buy submission.
Terminal output is routed through `WorkflowReporter`, which keeps live concurrent progress separate from final report tables. The pipeline returns structured run results; the final asset summary is sorted by original workflow index even when preparation completes out of order. Terminal tables intentionally show compact columns and wrap long messages, while CSV outputs retain the complete data. For example, the terminal Best Sharpe table is filtered to strategies with at least two trades and Sharpe of 1.0 or greater, and terminal Alpaca tables use chronological display IDs that preserve closed-position gaps while the CSV files keep raw database and broker identifiers.
A workflow timer is collected around the orchestration run and emitted at the output boundary as a compact footer followed by a divider for appended logs. The footer includes cumulative durations for market-data downloads, DB/state synchronization, compiled grid computation, report building/rendering/writes, and Alpaca reconciliation/submission, plus total wall time. Concurrent download calls are summed even when they overlap, so phase durations are diagnostic work time rather than an additive partition of wall time. Grid computation is timed around `run_grid_summary`; DB/state time covers the surrounding immediate transaction, including commit, with the kernel duration removed.
