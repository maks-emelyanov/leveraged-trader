...
7. Authenticate completed asset/signal pairs against the requested configuration and grid, then summarize the best strategy per asset. A shared-history update during later asset processing can invalidate an earlier completed pair. If reporting detects missing state, a configuration/grid mismatch, or a completed pair absent from the summary, orchestration rebuilds the affected pair from the same run's cached market-data snapshot. Each pair gets at most one recovery attempt, using its workflow side's original entry rule and grid. Recovery replaces its run result and restarts report generation for all workflow sides so earlier reports cannot survive a later rebuild. Recovery honors the existing workflow deadline; a failed rebuild, repeated invalidation, or expired deadline aborts before recommendation submission. Other reporting errors propagate without this recovery.
...
- filled or partially filled managed buys submit a GTC limit sell at the actual average fill price multiplied by the original sell multiple when Alpaca's wash-trade rules allow it; later parent-buy fills replace the active sell when its covered quantity or target changes, including after an earlier partial-fill sell completed. Final submission checks defer a new sell while an open same-asset buy could cross its limit, replacement checks preserve an existing sell until that conflict clears, and a definitive Alpaca wash-trade rejection returns the deterministic sell generation to a retryable state. These deferrals are required reconciliation failures, so the current run exits nonzero and a later recurring run retries after the conflict clears;
...
- the workflow retries historical-sell cancellations once after 10 seconds, in both full and reconciliation-only runs, when reconciliation classifies them as the only required protection or fill-accounting failures. Successful actions for other positions do not suppress the retry; independent failures do. The second pass opens a new database connection and repeats symbol migration, broker observations, identity, quantity, and exposure checks for all active managed positions. Completed first-pass actions remain in the final audit with messages marked as initial reconciliation. On retry failure or interruption, the audit retains all initial observations alongside the fresh failure diagnostics, and the workflow exits nonzero;
...
- managed sell fills are accumulated by Alpaca order; realized P/L uses matched quantity, and a position closes only after cumulative fills exactly cover the full managed buy quantity. Fresh and replacement protective orders require the broker's live holding to equal the managed remaining quantity, so unrecognized corporate actions or discretionary quantity changes fail closed instead of submitting incorrect coverage. A canceled pre-split GTC sell is recovered only after an ex-date-matched Alpaca reverse-split announcement and inactive-to-tradable stable-asset transition agree with either settled economics or Alpaca paper's exact unchanged pre-split accounting; reconciliation atomically rekeys and rebases the unsold position, scales its frozen target, and submits whole-share replacement protection. For the paper-only artifact it persists a fixed phantom-excess offset, and subsequent quantity fences accept only adjusted remaining quantity with or without exactly that offset. Realized-P/L validation treats the broker-proven rebased cost basis as the continuation of the immutable buy intent, while still requiring it not to exceed that intent's maximum cost. Fractional results stay quarantined unless the successor asset is explicitly non-fractionable, in which case the sub-share entitlement is durably recorded for broker cash-in-lieu settlement and only the whole tradable quantity is protected; an all-fractional outcome remains fail-closed, and unresolved cash-in-lieu keeps a later closed row out of complete realized P/L. Incomplete fill metadata, fill regressions, and overfills also fail closed for review. Managed rows that predate the inverse/Short workflow and its persisted workflow column are migrated to the only workflow that existed when they were opened, `Long`, so their realized P/L is not reported under an unknown side;
