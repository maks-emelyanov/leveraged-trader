...
- reconciliation fails nonzero, after persisting its result CSVs, whenever any current active managed position cannot confirm required protection or safe fill accounting. This includes broker authentication, availability, or transport failures, malformed or rejected protective-order responses, live-position quantity drift, and broker identity or order-metadata mismatches;
