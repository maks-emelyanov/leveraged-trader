...
- submitted buys use an explicitly capped batch cash allocation, equally split only among recommendations that pass live preflight and quote checks, as whole-share day limit orders with a configurable protective price buffer. Limit prices use four decimals below `$1` and two at or above `$1`; buys round down to their cap and sells round up to their target;
